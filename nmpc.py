...
3.) Decompresses (bz2 format) T3 data message (still raw binary)
4.) Creates a folder for individual events with following name format
GPSTIME_MICROSECOND
Example: 1117640005_616863
5.) Places output of x2 program, which is an ASCII text file containing the
3PMT traces (presumably mV levels for dynode/anode?)
"""
...
def save_gps(bytes,si):
    ...
    si += 4
    block = ['Current100','Next100','Current40','Next40','PreviousST','CurrentST','NextST']
...
def make_plots(evt_num,gps):
    ...
    for i in range(3):
        plt.subplot(1,3,i+1)
        plt.plot(xaxis,fadc_hist[:,i+1], drawstyle='steps-pre')
    ...
    for i in range(3):
        plt.subplot(1,3,i+1)
        plt.plot(xaxis,fadc_hist[:,i+4], drawstyle='steps-pre')
        plt.title('PMT {}'.format(i+1))
        plt.xlabel(r'Time [25 ns]')
        plt.ylabel('ADC counts')
    plt.tight_layout()
...
# Make Signal plot
    ...
    for i in range(3):
        ...
        xnew = xaxis[d_start-10:d_end+10]
        plot_y = y_sig[d_start-10:d_end+10] * y_peak_in_vem / (qvem_peak / 32)
        plot_y2 = y2_sig[d_start-10:d_end+10] * y2_peak_in_vem / qvem_peak
        #Add isolated signal window to list
        #Format: [ANODE1,DYNODE1,ANODE2,DYNODE2,ANODE3,DYNODE3]
        signal_only_trace.append(y_sig[a_start:a_end] / (qvem_peak / 32))
        signal_only_trace.append(y2_sig[d_start:d_end] / qvem_peak)
        #Plot anode
...
#A1_RISET D1_RISET A2_RISET D2_RISET A3_RISET D3_RISET
    aop_rt_array = np.zeros((2,6))
    for i in range(6):
        #np_signal_only_trace[:,i] = signal_only_trace[i]
        np.savetxt("signal_%i.txt" %i, signal_only_trace[i],fmt="%.2f")
        #Calculate area over peak and rise time
        max_time_bin = signal_only_trace[i].argmax()
        max_time = max_time_bin * 25
        aop_peak = signal_only_trace[i][max_time_bin] / max_time
        curr_signal = signal_only_trace[i].sum()
        aop = curr_signal / aop_peak
        signal_cdf = signal_only_trace[i].cumsum()
        #Find closest point that gives 10%
        signal_10pct = np.abs(signal_cdf - 0.1 * curr_signal)
        i10 = signal_10pct.argmin()
        if signal_cdf[i10] > 0.1 * curr_signal:
            xp = np.array([i10-1,i10])*25
            fp = np.array([signal_cdf[i10-1],signal_cdf[i10]])
            t10 = int(np.interp(curr_signal * 0.1,fp,xp))
        else:
            xp = np.array([i10,i10+1])*25
            fp = np.array([signal_cdf[i10],signal_cdf[i10+1]])
            t10 = int(np.interp(curr_signal * 0.1,fp,xp))
        #Find closest point that gives 50%
        signal_50pct = np.abs(signal_cdf - 0.5 * curr_signal)
        i50 = signal_50pct.argmin()
        if signal_cdf[i50] > 0.5 * curr_signal:
            xp = np.array([i50-1,i50])*25
            fp = np.array([signal_cdf[i50-1],signal_cdf[i50]])
            t50 = int(np.interp(curr_signal * 0.5,fp,xp))
        else:
            xp = np.array([i50,i50+1])*25
            fp = np.array([signal_cdf[i50],signal_cdf[i50+1]])
            t50 = int(np.interp(curr_signal * 0.5,fp,xp))
        rise_time = t50-t10
        aop_rt_array[0,i] = aop
        aop_rt_array[1,i] = rise_time
    np.savetxt("aop_risetime.txt",aop_rt_array,fmt="%.3f")
...
sp.call(['cp',"/home/augta/data/coincidence/%s.CTAG.gz" %fname,'/var/www/html/monitor/data/global_south/%s' %fname])
sp.call(['cp',"/home/augta/data/coincidence/%s.CTAL.gz" %fname,'/var/www/html/monitor/data/local_south/%s' %fname])

sp.call(['cp',"/home/augta/data/coincidence/%s.CTAG.gz" %fname,'.'])

global_gps = np.loadtxt('%s.CTAG.gz'%fname,usecols=(6,),dtype='S500',comments=None)
