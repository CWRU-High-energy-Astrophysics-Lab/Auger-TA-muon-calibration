...
def save_mon(bytes,si):
    ...
    si += 8220
    if len(bytes) < si + 4:
        raise ValueError("Not enough data to unpack 1")
...
with open('/media/hea/rootfs/home/que/Downloads/T3_for_Que/t3/2023_04_07 after reboot.T3','r') as t3file:
    copy = False
    for line in t3file:
        if line.strip() == "Event ...":
            copy = True
...
for t3 in t3list:
    ...
    f.write(dec_t3)
    f.close()   
    subprocess.call(["/media/hea/rootfs/home/que/Downloads/T3_for_Que/a.out", "T3_{}.bin".format(evt_id)])
    print("calling X2.c file")
